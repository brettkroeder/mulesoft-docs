...
|`${app}` |The name of the application which is triggering the alert. |All
|`${severity}` | The severity of the notification triggering the alert. |All
|`${period}` | Time period value to measure. |Period-based alerts
|`${periodTimeUnit}` | Time period unit of measure for the alert. |Period-based alerts
|`${threshold}` | Threshold set for the alert. |Period-based alerts
...
Not all properties are valid for all alerts.
