...
[NOTE]
The communication between a mule instance and the Anypoint Platform uses TLS 1.2 Mutual Exclusion, meaning the platform only trusts mule agents that have a signed certificate and the only truststore the agent uses is the platform public key.

It's also important to note that it is the mule instance that calls out to the Anypoint Platform, the Anypoint Platform does not initiate communication with a mule instance.
